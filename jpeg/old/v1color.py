...
rgb = imageio.imread('gshinchanbuff.png').astype(np.float32)
...
Y_padded, orig_h, orig_w = pad_image(Y, block_size=8)
Cb_padded, _, _ = pad_image(Cb, block_size=8)
Cr_padded, _, _ = pad_image(Cr, block_size=8)
...
# Paso 4: IDCT 2D manual (tipo III)
...
# Paso 6: Procesamiento por bloques
def process_channel(channel, Q):
    ...
    for i in range(0, h, block_size):
        for j in range(0, w, block_size):
            ...
            dct_block = dct_2d_manual(block - 128)
            quantized = np.round(dct_block / Q)
            dequantized = quantized * Q
            idct_block = idct_2d_manual(dequantized) + 128
...
# Paso 7: Recorte a tamaño original
...
plt.imsave('chimchamcolor.jpg', final_rgb)
